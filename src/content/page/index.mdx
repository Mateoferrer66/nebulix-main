---
title: VersKod la mejor Landing Page del mundo
description: "A Universe of Possibilities: Blogs, Portfolios, E - Commerce , Online
  Store, and Proyects"
intro: "A Universe of Possibilities: Blogs, Portfolios, E - Commerce , Online
  Store, and Proyects"
hero_template: image
template: full
container: md
hero_buttons:
  - component: Link
    label: Leer Más
    href: "#more"
    class: surface-primary
    icon_only: false
    icon: /src/icons/down-line.svg
    color: surface-primary
  - component: Link
    label: docs
    href: /blog/tag/docs
    class: optional
    icon_only: false
    icon: /src/icons/page.svg
pubDate: Jul 08 2022
layout-hidden: blog
blocks:
  - type: features
    component: Features
    container: lg
    block_class: " py-20 text-center"
    features:
      - content: >
          ### Food IA 


          Aplicación IA que de manera automatica seleccione la preferencioa del almuerzo a una hora especifica por persona 
          Adicional cubre una problematica que es el tener que seleccionar la comida y mas cuando no hay tiempo. La aplicacion brindara 3 tipos de almuerzos
          o propuestas de almuerzo por medio de una notificación,sera 100% automatico y autonomo y con domicilio incluido. 

          Creado por Angello 

        icon: /src/icons/tailwind.svg
        color: bg-[#38bdf8] text-white
      - color: bg-[#fc0] text-black
        icon: /src/icons/shop.svg
        content: >
          ### Webshop


          Aplicacion IA ayuda a los medicos a dar diagnosticos mas precisos basados en informacion proporcionada por el historial medico, 
          donde infroma las ventajas y desventajas de cualquier medicamento, resolucion de dudas de salud en general 
          y como evitar enfermedades comunes segun factores de localizacion y edad. resolviendo la causa del problema y en aliviar sintomas,
          Es una combinacion de un nuevo enfoque de medicina y tecnologia que permiten un sistema de salud mas efectivo y al alcance de aquellos
          que posean al menos un movil o conexion a internet. 

      - icon: /src/icons/blocks-group-solid.svg
        content: >
          ### Chatbot IA

          Se creara un aplicacion de chatbot como asistente virtual para las empresas o Landing page que lo requieran para agilizar y mostrar mejor la informacion que se quiere brindar >
          al cliente por parte de la inteligencia artificial, facilitar el servicio al cliente personalizado y adicional mostrar mejor la informacion de la pagina web. 
          
        color: surface-secondary
        href: ""
      - content: >
          ### Full-Text Search


          Experience lightning-fast full-text search capabilities with Pagefind. Our integrated search engine seamlessly enhances your site's user experience.
        icon: /src/icons/book-search-24-regular.svg
        color: surface-accent
      - icon: /src/icons/template.svg
        content: >
          ### Multiple Templates


          Tailor your website to perfection with a variety of templates for pages, blogs, shops, portfolios, and menus. Craft each page precisely as you envision.
        color: surface-success
        href: ""
      - icon: /src/icons/menu.svg
        content: >
          ### Restaurant Menu


          Explore our user-friendly menu, thoughtfully designed with clear labels and detailed information about allergens for your dining peace of mind.
        color: surface-warning
        href: ""
      - href: "#"
        icon: /src/icons/git.svg
        content: >
          ### Git-Based


          With Astro + Static CMS, your content finds its home in a Git repository, ensuring efficient content management and collaboration.
        color: surface-primary
      - content: >
          ### Light & Darkmode


          Give your visitors the freedom to choose their preferred color scheme, ensuring a personalized browsing experience.
        color: surface-dark
        icon: /src/icons/dark-theme-20-filled.svg
    content: |+
      ## Proyectos A la Medida 




    surface: bg-slate-200 text-dark dark:bg-slate-900 dark:text-white
  - type: text_image
    component: TextImage
    container: md
    template: column
    title: My Image
    image_size: "66"
    reverse: true
    animate: false
    block_class: ""
    thumbnail: /src/assets/perfect_pagespeed_scores.png
    content: >
      ## **A perfect page speed score** Optimized Speed and Accessibility


      Our perfect 100% page speed score and compliance with WCAG 2.1 AA standards reflect our commitment. We've optimized speed and accessibility, ensuring a superior user experience for all.
  - type: recentitems
    component: RecentItems
    page_type: blog
    count: 3
    container: md
    template: list
    animate: true
    post_tag:
      - Docs
    title: Check out our documentation
    surface: bg-slate-200 text-dark dark:bg-slate-800 dark:text-white
    content: >
      Getting started is easy, just clone the repo, configure and you are ready
      to go.
  - type: text_image
    component: TextImage
    container: none
    template: banner
    title: My Image
    reverse: true
    content: >
      ## Nebulix Webshop


      Discover the seamless integration of Snipcart, empowering you to efficiently oversee your store directly within a Static CMS. Whether your offerings include digital products or physical goods, Snipcart boasts a wealth of exceptional features, making it an ideal choice for the majority of small webshops. 


      Read more about [SnipCart](https://snipcart.com/).
    thumbnail: /src/assets/nebulix_merch.jpeg
    buttons:
      - icon_only: false
        label: Demo Shop
        href: /shop
        icon: /src/icons/shop.svg
        color: surface-primary
    aspect: ""
    image_size: ""
    animate: false
    block_class: my-0
  - type: rich_text
    component: RichText
    container: sm
    content: >
      ## Nebulix Portfolio


      Need to show your skills, select a good template and build your portfolio with nebulix.
    buttons:
      - icon_only: false
        label: Portfolio Demo
        href: /work
        color: surface-primary
        icon: /src/icons/portfolio.svg
    block_class: py-20 text-center
  - type: text_image
    component: TextImage
    container: none
    template: banner
    image_size: ""
    title: My Image
    reverse: false
    animate: false
    thumbnail: /src/assets/cosmic_coctails_3.jpeg
    content: >+
      ## Nebulix Restaurant Menus


      Create a great menu, you can even shop arlercic information and labels. All menu's are mobile friendly and fast!








    buttons:
      - icon_only: false
        label: Demo Menu
        color: surface-primary
        icon: /src/icons/menu.svg
        href: /menu/drinks
        class_name: py-0
    block_class: my-0
style:
  hero_template: image
  template: full
  container: sm
  content_class: py-20 text-center
  page_class: ""
  card_template: grid
  hero_surface: surface-dark
  hero_image_opacity: ""
thumbnail: /src/assets/nemulala.jpg
og_image: /src/assets/og_image_nebulix.png
---

## Quienes Somos ? 
Un grupo de jovenes emprendedores con una idea readicada desde 2024 
contamos con personas emprendedoras de mas de 8 paises, 
que desean posicionarse profesionalmente, adquirir experiencia,
compartir y aplicar sus conocimientos por medio de startups y proyectos, 
generando herramientas útiles para agilizar la productividad de las personas 
y procurar una mejor calidad de vida.